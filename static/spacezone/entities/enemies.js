...
room.registerElement('spacezone-enemy-mine', {
  ...
  explosionThreshold: 60, // Explosion distance in meters
  ...
  explodeMine() {
    ...
    const distance = this.distanceTo(this.player);
    const damageAmount = Math.max(this.explosionThreshold - Math.pow(distance / 4, 2), 0); // Ensure damage is not negative
