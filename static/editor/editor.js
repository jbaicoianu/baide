// /* 
// CSS for Summarize Project Feature:
// #summarizeProjectBtn {
//   margin-top: 10px;
//   padding: 8px 12px;
...
//   display: none;
// }
// #summarizeDialog.active {
//   display: block;
// }
// #summarizeDialog h3 {
//   margin-top: 0;
// }
// #summarizeDialog .file-list {
//   max-height: 300px;
//   overflow-y: auto;
...
//   margin-bottom: 20px;
// }
// #summarizeDialog .file-item {
//   display: flex;
//   align-items: center;
//   margin-bottom: 5px;
// }
// #summarizeDialog .file-item input {
//   margin-right: 10px;
// }
// #summarizeDialog .buttons {
//   display: flex;
//   justify-content: flex-end;
//   gap: 10px;
// }
// #summarizeDialog .buttons button {
//   padding: 8px 12px;
//   border: none;
//   border-radius: 4px;
//   cursor: pointer;
// }
// #summarizeDialog .buttons .summarizeBtn {
//   background-color: #28a745;
//   color: white;
// }
// #summarizeDialog .buttons .summarizeBtn:hover {
//   background-color: #218838;
// }
// #summarizeDialog .buttons .cancelBtn {
//   background-color: #dc3545;
//   color: white;
// }
// #summarizeDialog .buttons .cancelBtn:hover {
...
async function loadProjectStructure() {
  try {
    ...
    if (response.ok) {
      ...
      // Add Summarize Project Button
      const summarizeProjectBtn = document.createElement('button');
      summarizeProjectBtn.id = 'summarizeProjectBtn';
      summarizeProjectBtn.textContent = 'Summarize Project';
      summarizeProjectBtn.addEventListener('click', openSummarizeDialog);
      projectBrowser.appendChild(summarizeProjectBtn);
...
function setupEventListeners() {
  ...
  // Add event listener for Summarize Project dialog buttons
  const summarizeDialog = document.getElementById('summarizeDialog');
  if (summarizeDialog) {
    const summarizeBtn = summarizeDialog.querySelector('.summarizeBtn');
    const cancelBtn = summarizeDialog.querySelector('.cancelBtn');
    summarizeBtn.addEventListener('click', handleSummarizeProject);
    cancelBtn.addEventListener('click', closeSummarizeDialog);
  }
...
// Function to open Summarize Project dialog
function openSummarizeDialog() {
  const overlay = document.getElementById('overlay');
  const dialog = document.getElementById('summarizeDialog');
  overlay.classList.add('active');
  dialog.classList.add('active');
  populateSummarizeDialog();
}

// Function to close Summarize Project dialog
function closeSummarizeDialog() {
  ...
  summarizeContent.innerHTML = '';
}

// Function to populate the Summarize Project dialog with file selection
function populateSummarizeDialog() {
  ...
  // Recursive function to create file tree with checkboxes
  function createFileCheckboxes(structure, parentElement, currentPath = '') {
    structure.forEach(item => {
      if (item.type === 'file') {
        ...
        checkbox.value = currentPath + item.name;
        checkbox.id = `summarize-${sanitizeId(currentPath + item.name)}`;
        
        const label = document.createElement('label');
        label.htmlFor = `summarize-${sanitizeId(currentPath + item.name)}`;
        ...
        dirLabel.textContent = item.name;
        dirItem.appendChild(dirLabel);
        
        const subList = document.createElement('div');
        subList.style.marginLeft = '20px';
        createFileCheckboxes(item.children, subList, currentPath + item.name + '/');
        dirItem.appendChild(subList);
  ...
  // Fetch project structure again to populate dialog
  fetch('/project_structure')
    .then(response => response.json())
    .then(data => {
      createFileCheckboxes(data, fileList);
      summarizeContent.appendChild(fileList);
    })
    .catch(error => {
      console.error('Error fetching project structure for summarization:', error);
      summarizeContent.textContent = 'Error loading files.';
    });
  ...
  summarizeContent.appendChild(buttonsDiv);
}

// Function to handle summarizing the selected project files
function handleSummarizeProject() {
  const checkboxes = document.querySelectorAll('#summarizeDialog .file-item input[type="checkbox"]:checked');
  const selectedFiles = Array.from(checkboxes).map(cb => cb.value);
  
  if (selectedFiles.length === 0) {
    showToast('Please select at least one file to summarize.', 'error');
    return;
  }
  
  // Disable buttons to prevent multiple submissions
  const summarizeBtn = document.querySelector('#summarizeDialog .summarizeBtn');
  const cancelBtn = document.querySelector('#summarizeDialog .cancelBtn');
  ...
  summarizeContent.appendChild(loadingDiv);
  
  // Send POST request to /summarize_project
  fetch('/summarize_project', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  .then(data => {
    ...
    if (data.error) {
      summarizeContent.textContent = `Error: ${data.error}`;
    } else if (data.summary) {
      const summaryDiv = document.createElement('div');
      summaryDiv.textContent = data.summary;
      summarizeContent.appendChild(summaryDiv);
    } else {
      summarizeContent.textContent = 'No summary available.';
    }
  ...
  .catch(error => {
    ...
    summarizeContent.innerHTML = 'Error summarizing project.';
    // Re-enable buttons
    summarizeBtn.disabled = false;
    cancelBtn.disabled = false;
  });
...
function closeTab(filename) {
  const sanitizedId = sanitizeId(filename);
  const tab = document.getElementById(`tab-${sanitizeId(filename)}`);
  if (tab) {
    tab.parentNode.removeChild(tab);
    delete openFiles[filename];
    saveOpenFiles();
...
window.onclick = function(event) {
  ...
  const summarizeDialog = document.getElementById("summarizeDialog");
  const overlay = document.getElementById("overlay");
  if (event.target == overlay && summarizeDialog.classList.contains('active')) {
